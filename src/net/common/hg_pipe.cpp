...
#define MAX_PIPE_SIZE 64*1024
#define MAX_BUF_BLOSIZE 512

HgPipe::HgPipe() {
    fragCache = myPoolCreate(MAX_PIPE_SIZE);
    _spin_init(&lock, 0);
    lock = 0;
    uint8_t datachars[MAX_PIPE_SIZE];
    tcpbuf.data = datachars;
    tcpbuf.cap = MAX_PIPE_SIZE;
    int re = pipe(pipe_fd);
    MY_NOBLOCK(pipe_fd[0]);
    MY_NOBLOCK(pipe_fd[1]);
    if (re == -1) {
        ALOGI(0, "create pipe invalid");
    }
}
...
void HgPipe::PipeSendmsg(void *ioctx, void *ctx, void *ptr) {
    ...
    hg_chain_node *hcnleft = hp->hct.left;
    hg_chain_node *hcnleftnxt = nullptr;
...
int HgPipe::Handlemsg(int pfd, hg_Buf_Gen_t *tcpb) {
    int n = 0;
    uint8_t *recvLine = nullptr;
    int error = 0;

    uint8_t body[MAX_PIPE_SIZE];
    uint8_t *tmp = nullptr;
...
int HgPipe::WritePipe0(HgPipe *hp, void *data, int size) {
    ...
    //ALOGI(0, "rrrrrrrrrrrrrrrrrrrrrrrrr 80-8");
    n = write(hp->pipe_fd[1], (char *) data, size);
    //ALOGI(0, "rrrrrrrrrrrrrrrrrrrrrrrrr 80-8 %d", n);
    if (n >= 0) {
        return n;
        //return n;
    }
