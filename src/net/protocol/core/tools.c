...
#define seq_max_NUM 65535//不是最大的值而是总共的个数，0-65534
void init16Num(){
    // seq_max_NUM=65536;//(65535-(65535%sess_cache_LEN));
}

int uint16Sub(uint16_t newd,uint16_t oldd){
    uint32_t ret;
    if(newd<oldd){
        ret= (newd-0)+1+(seq_max_NUM-1)-oldd;
        if(ret>seq_max_NUM/2){
            return -1;
        }else{
            return ret;
        }
...
uint16_t uint16Add(uint16_t add1,uint16_t add2){
    return (uint16_t)((add1+add2)%(seq_max_NUM));
}
...
int hg_pthread_mutex_lock(const char *file,uint32_t line,const char *fun,pthread_mutex_t *pmt){
    uint32_t ret;
    ret=pthread_mutex_lock(pmt);
    if(ret!=0){
        ALOGI(0,"lockerr %s %d %s\n",file,line,fun);
    }
    return ret;
}
int hg_pthread_mutex_unlock(const char *file,uint32_t line,const char *fun,pthread_mutex_t *pmt){
    uint32_t ret;
    ret=pthread_mutex_unlock(pmt);
    if(ret!=0){
        ALOGI(0,"unlockerr %s %d %s\n",file,line,fun);
    }
