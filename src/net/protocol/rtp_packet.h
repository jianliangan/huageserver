...
#define TCP_PACKET_HEAD_SIZE 4
#define ADD_PACKET_HEAD1_SIZE 8
#define MAX_RTP_PACKET_SIZE 1000  //保证每个包可以整除4，512-12=500

#define RTP_PACKET_HEAD_SIZE 10


//标识做什么的消息，即消息类型，鉴权的两类消息是低级的，不需要验证权限，每一种类型代表一种处理流程
enum CMDTYPE {
    CMD_DEFAULT, RTP_CMD_POST, RTP_CMD_RES_REPOST, RTP_CMD_REQ_REPOST, RTP_CMD_TCP
};
 ...
 */
    ...
    rtpHeader():first(0),comp(0),cmd(0),tail(0),payloadType(0),seq(0)
...
typedef struct localPacket {
    uint16_t size;
    uint8_t *data;
} localPacket;

uint8_t *rtpPacketHead(uint8_t *dst, rtpHeader *rh);
...
PLTYPE pktPLoadType(uint8_t *data, uint32_t size);
